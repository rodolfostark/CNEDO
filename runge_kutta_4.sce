function [t, y, z] = runge_kutta_4(a, b, h, y0, z0)
    ...
    for i=2:1:n
        ...
        k1y = dy(t(i-1), y(i-1), z(i-1));
        k1z = dz(t(i-1), y(i-1), z(i-1));
        //segundo vamos calcular as inclinações k2z e k2y
        k2y = dy(t(i-1) + h/2, y(i-1) + k1y*h/2, z(i-1) + k1z*h/2);
        k2z = dz(t(i-1) + h/2, y(i-1) + k1y*h/2, z(i-1) + k1z*h/2);
        //terceiro vamos calcular as inclinações k3z e k3y
        k3y = dy(t(i-1) + h/2, y(i-1) + k2y*h/2, z(i-1) + k2z*h/2);
        k3z = dz(t(i-1) + h/2, y(i-1) + k2y*h/2, z(i-1) + k2z*h/2);
        //quarto vamos calcular as inclinações k4ye k4z
        k3y = dy(t(i-1) + h, y(i-1) + k3y*h, z(i-1) + k3z*h);
        k3z = dz(t(i-1) + h, y(i-1) + k3y*h, z(i-1) + k3z*h);
